class Anamoly:
    ...
    def compute(X,Y,X1,Y1,LOOKBACK_SIZE, num_of_numerical_features:int, epocs:int, MODEL_SELECTED=MODEL_SELECTED, KERNEL_SIZE=KERNEL_SIZE):
        """
        ...
        if str(MODEL_SELECTED) == "lstmaenn":
            ...
            hypothesis = loaded_model(torch.from_numpy(X1.astype(np.float32)).to(device)).detach().cpu().numpy()
            loss = np.linalg.norm(hypothesis - Y1, axis=1)
            return loss.reshape(len(loss),1), train_data, model
        else:
